...
The `rconf` `podman` module allows you to manage Podman containers and images on remote systems. It provides functionalities to run, stop, and remove containers, as well as pull or build images. Because Podman is a daemonless container engine, this can be an excellent alternative to Docker.
...
*   **`build_path`**: Path to a directory containing a `Containerfile` or `Dockerfile`. If provided with `action: run`, an image will be built and tagged with the value of the `image` parameter (or `name` if `image` is not set), then the container will be started from it.
...
*   **`force`**: For `action: remove`, forces the removal of the container (`podman rm -f`). Defaults to `false`.
...
**Example:**
```yaml
- name: "Run an Nginx container with Podman"
  rconf:podman:
    name: "my-nginx"
    image: "nginx:alpine"
    ports:
...
Builds a Podman image from a local path, then runs a container from that image.
...
Ensures a specific image is pulled onto the host.
